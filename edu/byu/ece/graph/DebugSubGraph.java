...
public class DebugSubGraph {
	...
	public static void addNeighbors(Object node, int depth, 
			BasicGraph graph, Collection neighbors,
			boolean pred, boolean succ) {
		neighbors.add(node);
	...
	public static Collection getNeighbors(EdifCellInstance eci, int depth, 
			BasicGraph graph, boolean bidirectional) {
	...
	public static AbstractEdifGraph getSubGraph(String eciName, int depth, 
			EdifCell cell, boolean bidirectional, boolean flatten) throws InvalidEdifNameException {

		// Flatten design
		if (flatten) {
			if (DEBUG) System.out.println("Flattening top level EdifCell...");
		...
		if (DEBUG) System.out.println("Searching for instance "+eciName+"...");
		EdifCellInstance eci = cell.getCellInstance(eciName);
		if (eci == null) {
			if (DEBUG) System.out.println("Could not find EdifCellInstance: "+eciName);
			return null;
		}
		
		// Create the graph representing the EdifCell
		EdifCellInstanceGraph graph = 
			new EdifCellInstanceGraph(cell, true);
	...
	public static void main(String[] args) {
		...
		if (eciName == null) {
			System.out.println("USAGE: java DebugSubGraph <inputEdifFile> " +
					...
					"[-o output dotty file] [-bi](bi-directional search) " +
					"[-nf](don't flatten circuit)");
			System.exit(-1);
		}
        ...
        try {
            subGraph = getSubGraph(eciName, depth, cell, 
