...
public class HalfLatchReplacer {
	...
	public static EdifEnvironment replaceHalfLatches(EdifEnvironment env) {
		...
		System.out.println("****************************************");
		int ffCount = 0;	// Used for naming new FDCPE instances
		for (ReplacementContext context : ecr.getReplacementContexts()) {
			...
			//edu.byu.ece.edif.tools.sterilize.halflatch.HalfLatchReplacement.Replace(newLibManager, topCell, newEnv,
					//oldCell.getName(), newParent, oldInstance.getName(), INIT, C, D, Q, PRE, CE, CLR, ffCount);
