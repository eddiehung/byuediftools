...
public abstract class EdifHalfLatchRemover {
	...
	public EdifHalfLatchRemover(HalfLatchArchitecture hlArchitecture,
			int safeConstantPolarity, boolean usePortForConstant) {
	 ...
	 *        safe-constant network is driven through a port or internal
	 *        constant generator cells
	 * @param safeConstantPortName The name of the port to use for the safe
	 *        constant.
	 */
	public EdifHalfLatchRemover(HalfLatchArchitecture hlArchitecture,
			int safeConstantPolarity, boolean usePortForConstant, 
			String safeConstantPortName) {
	...
	public HalfLatchFlattenedEdifCell removeHalfLatches(NewFlattenedEdifCell flatCell) {
		...
		try {
			// Create new HLFlattenedCell in a new Library so the name doesn't have to change?
			hlFlatCell = new HalfLatchFlattenedEdifCell(flatCell, 
					_hlArchitecture, _safeConstantPolarity);
		}
	...
	public EdifEnvironment removeHalfLatches(EdifEnvironment environment) {
		...
		EdifCell top_hl_cell = convertCelltoHalfLatchCellDeep(
				environment.getTopCell(), null, new_env.getLibraryManager(), 
				_hlArchitecture, _safeConstantPolarity);
	...
	protected void setTopDesignAndInstance(EdifEnvironment new_env, HalfLatchEdifCell hl_top_cell, EdifEnvironment environment) {
		...
		if (old_design.getPropertyList() != null) {
			for (Iterator it = old_design.getPropertyList().values().iterator(); it
					.hasNext();) {
	...
	public static EdifCell convertCelltoHalfLatchCellDeep(EdifCell cellToConvert,
		...
		try {
			hlCell = new HalfLatchEdifCell(targetLib, 
					cellToConvert.getEdifNameable(), 
					hlArchitecture, safeConstantPolarity);
		} catch (EdifNameConflictException e) {
			e.toRuntime();
		}
		...
		if (cellToConvert.getPropertyList() != null) {
			for (Iterator it = cellToConvert.getPropertyList().values().iterator(); it
					.hasNext();) {
				Property p = (Property) it.next();
		...
		if (cellToConvert.getInstancedCellTypes() != null) // there are subcells to
			// copy
			for (Iterator instanced = cellToConvert.getInstancedCellTypes()
				...
				EdifCell newCellRef = convertCelltoHalfLatchCellDeep(cell, null,
						elm, hlArchitecture, safeConstantPolarity);
		...
		for (Iterator<EdifCellInstance> instanceIterator = (Iterator<EdifCellInstance>) cellToConvert
			...
			if (oldEci.getCellType().isPrimitive() && 
					hlArchitecture.cellRequiresReplacement(oldEci)) {
				...
				try {
					newInstance = new EdifCellInstance(oldEci.getName()
							+HalfLatchEdifCell.SENSITIVE_SUFFIX, hlCell, newCellRef);
				}
				catch (InvalidEdifNameException e) {
					// This shouldn't happen, but if it does, just use the 
					// original name.
					if (_debug) System.out.println("WARNING: Sensitive primitive "
							+oldEci+"could not be renamed to "+oldEci.getName()
							+HalfLatchEdifCell.SENSITIVE_SUFFIX);
					newInstance = new EdifCellInstance(oldEci
							.getEdifNameable(), hlCell, newCellRef);
				}
			}
			else {
		...
		for (EdifPort oldPort : cellToConvert.getPortList()) {
			...
			try {
				newPort = hlCell.addPort(oldPort.getEdifNameable(),
		...
		// copy nets
		for (Iterator<EdifNet> netIterator = (Iterator<EdifNet>) cellToConvert
	...
	public static void main(String[] args) {
        ...
        if (usePortForConstant == true) System.out.println ("Using top-level port for safe constant source");
        else System.out.println ("Using internal cell(s) for safe constant source");        
        
        // Decide what type of half-latch removal to do... sequential or recursive
        ...
        if (constantSinkThreshold != -1 && usePortForConstant == true)
        	throw new EdifRuntimeException("Conflicting arguments.  Cannot use top-level port to drive safe constant network AND have internal constant generator cells.  Conflicting argument flags are -useportforconstant and -constantsinkthreshold.");
        ...
        // or if the user only wants *one* internal safe constant generator source,
        // then use more simple Sequential half-latch removal algorithm.
        // Otherwise (the user wants to have multiple internal constant generator sources)
        // use the Topological half-latch removal algorithm.
        if (constantSinkThreshold == -1 || usePortForConstant == true) {
        	System.out.println("Using Sequential algorithm...");
        	edifHalfLatchRemover = new SequentialEdifHalfLatchRemover(hlArchitecture, safeConstantPolarity, usePortForConstant);
        } else {	
        	System.out.println("Using Topological algorithm...");        	
        	edifHalfLatchRemover = new TopologicalEdifHalfLatchRemover(hlArchitecture, safeConstantPolarity, usePortForConstant, constantSinkThreshold);
        }
        ...
        // then write the modified edif to file.  Default filename
        // is hl.edf
