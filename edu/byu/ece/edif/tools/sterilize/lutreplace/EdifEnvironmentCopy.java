...
/**
 * This class copies an EdifEnvironment and maintains an association between the objects
 * in the old EdifEnvironment and the objects in the new EdifENvironment.
 * 
...
public class EdifEnvironmentCopy {
	...
	/**
	 * Initiates the copying of the old environment and the
	 * creation of a new environment. This will perform a 
	 * "deep" copy and copy all elements needed for the top-level
	 * design. 
	 * 
	 * The name of the new environment will be assigned to
	 * the name of the old environment.
	 * 
	 * @return
	 ...
	 * - Copy the properties of the environment
	 * - Copy Date, Author, Program, and Version of Environment
	 * - Create the top-level design and all EdifCells needed for design
	 *   (recursive call)
	 * 
	 * @return
	 * @throws EdifNameConflictException
	 ...
	 * the new environment. These parameters are not immutable so they can
	 * be changed at a later time.
	 * 
	 * This method can also be extended to change the default behavior. 
	 */
	 ...
	 * - Copies the top-level EdifCell that represents the top-level design
	 *   (it does this recursively and will thus add all the libraries, cells,
	 *    etc. for this top-level Cell)
	 * - Creates new top-level instance
	 * - Creates new EdifDesign
	 * - Copies EdifDesign properties
	 * 
	 * @throws EdifNameConflictException
	 */
	protected void createTopDesign() throws EdifNameConflictException {
		...
		EdifCellInstance newTopInstance = 
			new EdifCellInstance(newTopCell.getEdifNameable(), null, newTopCell);
	...
	/**
	 * Copy the given EdifCell from the original Environment and add it to 
	 * the new environment. Create all the libraries and children cells
	 * necessary to build the EdifCell.
	 * 
	 * This method performs the following steps:
	 * - Create new cell and place in map
	 * - Add ports to cell
	 * - add instances to cell (including sub-cells through recursion)
	 * - add nets and wire them up.
	 * 
	 * @param origCell
	 */
	 ...
	 * creates an empty library. EdifCells are added to the library during
	 * the copyEdifCell methods.
	 *
	 * - Create new library
	...
	protected void addEdifPorts(EdifCell origCell, EdifCell newCell)
		...
		for (EdifPort oldPort : origCell.getPortList()) {
			EdifPort newPort = newCell.addPort(oldPort.getEdifNameable(),
	...
	protected void addChildEdifCellInstance(EdifCell origCell, EdifCell newCell, 
		...
		// See if the EdifCell of the instance is in the library. Add if necessary.
		...
		// Create instance
		EdifCellInstance newInstance = new EdifCellInstance(
				oldChildInstance.getEdifNameable(), newCell, newChildType);

		// copy instance properties
		newInstance.copyProperties(oldChildInstance);
		_instanceMap.put(oldChildInstance, newInstance);

		// TODO: This is odd: I don't think you should have to construct the instance
	...
	protected EdifNet addNet(EdifCell origCell, EdifCell newCell, EdifNet oldNet) 
		throws EdifNameConflictException {
		// TODO: again, this is odd that you create a net and don't automatically add it.
