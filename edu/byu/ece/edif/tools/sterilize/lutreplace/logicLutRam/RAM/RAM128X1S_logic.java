/*
 * Logic implementation of a RAM128X1S
 * 
...
public class RAM128X1S_logic extends Logic {
    public static CellInterface cell_interface[] = { in("WE", 1), in("D", 1), in("WCLK", 1), in("A0", 1), in("A1", 1),
            in("A2", 1), in("A3", 1), in("A4", 1), in("A5", 1), in("A6", 1), out("O", 1), };

    protected boolean cellInterfaceDeterminesUniqueNetlistStructure() {
        return true;
    }

    public RAM128X1S_logic(Node parent, Wire we, Wire d, Wire wclk, Wire a0, Wire a1, Wire a2, Wire a3, Wire a4,
            Wire a5, Wire a6, Wire o, int init1, int init2, int init3, int init4) {
        super(parent);
        ...
        for (int i = 0; i < WIDTH; i++) {
            if (((inits[i / MIN] >> i % MIN) & 1) == 1) {
                new fdpe(this, wclk, d, and(we, rdec.gw(i)), gnd(), regout.gw(i));
            } else {
                new fdcpe(this, wclk, d, and(we, rdec.gw(i)), gnd(), gnd(), regout.gw(i));
            }
