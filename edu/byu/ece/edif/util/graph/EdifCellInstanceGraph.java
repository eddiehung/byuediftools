...
public class EdifCellInstanceGraph extends AbstractEdifGraph {
    ...
    public void removeIOBufs() {
        ...
        /*
         * Iterate over all the nodes in the graph looking for "IBUFs", "BUFGPs"
         * and "OBUFs".
         */
        for (Object node : this.getNodes()) {
            Collection<EdifNet> netList = new LinkedHashSet<EdifNet>();

            if (node instanceof EdifCellInstance) {
                EdifCellInstance eci = (EdifCellInstance) node;
                EdifCell parent = eci.getParent();
                String cellName = eci.getCellType().getName().toUpperCase();

                /*
                 * An "IBUF" or a "BUFGP" is found. Grab the node's predecessor
                 * (there should only be one since it can't have more than one
                 * thing driving it). Also grab the node's sucessors (sinks).
                 * Remove the edges connecting to the node, and then remove the
                 * node. Create new edges from the predecessor to all sucessors.
                 */
                if ((cellName.startsWith("IBUF") || cellName.startsWith("BUFGP")) && eci.getCellType().isPrimitive()) {
                    EdifSingleBitPort source = (EdifSingleBitPort) this.getPredecessors(node).iterator().next();
                    Collection<EdifCellInstance> sinks = this.getSuccessors(node);
                    EdifCellInstanceEdge inEdge = (EdifCellInstanceEdge) this.getEdge(source, node);
                    this.removeEdge(inEdge);

                    /*
                     * Iterate over all the node's successors, creating new
                     * edges to each one. Also delete old edges from the node.
                     */
                    for (EdifCellInstance sink : sinks) {
                        EdifCellInstanceEdge outEdge = (EdifCellInstanceEdge) this.getEdge(node, sink);
                        this.removeEdge(outEdge);
                        EdifPortRef oldEPR = outEdge.getSinkEPR();
                        EdifPortRef newEPR = new EdifPortRef(inEdge.getNet(), oldEPR.getSingleBitPort(), oldEPR
                                .getCellInstance());
                        EdifCellInstanceEdge newEdge = new EdifCellInstanceEdge(inEdge.getSourceEPR(), newEPR);
                        EdifNet net = newEdge.getNet();

                        /*
                         * In addition to deleting and creating new edges in the
                         * graph, we need to update the EDIF net information.
                         */
                        if (!netList.contains(net)) {
                            netList.add(net);
                            net.deletePortConnection(inEdge.getSinkEPR());
                        }
                        net.addPortConnection(newEPR);
                ...
                /*
                 * An "OBUF" is found. Grab the node's predecessor and
                 * successor. There should only be one successor, and one
                 * predecessor. Remove the edges connecting to the node, and the
                 * node itself. Create a new edge from predecessor to successor.
                 * Also update the EDIF information.
                 */
                if (cellName.startsWith("OBUF") && eci.getCellType().isPrimitive()) {
                    EdifSingleBitPort sink = (EdifSingleBitPort) this.getSuccessors(node).iterator().next();
                    EdifCellInstance source = (EdifCellInstance) this.getPredecessors(node).iterator().next();
                    EdifCellInstanceEdge inEdge = (EdifCellInstanceEdge) this.getEdge(source, node);
                    EdifCellInstanceEdge outEdge = (EdifCellInstanceEdge) this.getEdge(node, sink);
                    this.removeEdge(inEdge);
                    this.removeEdge(outEdge);
                    this.removeNode(node);
                    EdifCellInstanceEdge newEdge = new EdifCellInstanceEdge(inEdge.getSourceEPR(), outEdge.getSinkEPR());
                    EdifNet net = newEdge.getNet();
                    net.deletePortConnection(outEdge.getSourceEPR());
                    net.addPortConnection(inEdge.getSourceEPR());
                    inEdge.getNet().deletePortConnection(inEdge.getSinkEPR());
                    Collection<EdifCellInstanceEdge> sourceOut = this.getOutputEdges(source);

                    /*
                     * The EdifNet structures have to be updated. All EdifNets
                     * that used to were associated with the OBUF input need to
                     * now be associated with the output EdifSingleBitPort. This
                     * means iterating through all edges coming from the
                     * 'source' and deleting the EdifNets associated with the
                     * edge, but adding the sink EdifPortRef to the EdifNet
                     * associated with the new edge. TODO: The EdifNet doesn't
                     * seem to be deleted from the parent cell. Thus the
                     * explicit removal of input/output EdifPortRefs. This
                     * leaves an empty EdifNet still in the EDIF.
                     */
                    for (EdifCellInstanceEdge srcOut : sourceOut) {
                        EdifNet delNet = srcOut.getNet();
                        parent.deleteNet(delNet);
                        Collection<EdifPortRef> portRefs = delNet.getInputPortRefs();
                        for (EdifPortRef epr : portRefs)
                            delNet.deletePortConnection(epr);
                        portRefs = delNet.getOutputPortRefs();
                        for (EdifPortRef epr : portRefs)
                            delNet.deletePortConnection(epr);
                        net.addPortConnection(srcOut.getSinkEPR());
                    }
                    this.addEdge(newEdge);
                }
            }
    ...
    public void toEdif(String filename) {
        ...
        try {
             ...
             */
            Collection<EdifCellInstanceEdge> nets = this.getEdges();
            for (EdifCellInstanceEdge link : nets) {
