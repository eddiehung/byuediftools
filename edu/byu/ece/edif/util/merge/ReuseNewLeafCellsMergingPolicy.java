/*
 * Reuse only EdifCells added during merging, preserving the original libraries.
 * 
...
/**
 * An EdifMergingPolicy which will try to reuse only EdifCells that have been
 * added during merging, preserving the libraries in the original file.
 */
        ...
        if (cellToCopy.isLeafCell()) {
            ...
            for (Iterator it = matchingNames.iterator(); it.hasNext();) {
                ...
                if (cellToCopy.equalsInterface(cell) && cellToCopy.equalsProperties(cell)
                        && mergeLibs.contains(cell.getLibrary())) {
                    lib = cell.getLibrary();
                    return lib;
                }
            }
