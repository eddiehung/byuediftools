/*
 * Logic implementation of a RAM64X1S_1
 * 
...
public class RAM64X1S_1_logic extends Logic {
    public static CellInterface cell_interface[] = { in("WE", 1), in("D", 1), in("WCLK", 1), in("A0", 1), in("A1", 1),
            in("A2", 1), in("A3", 1), in("A4", 1), in("A5", 1), out("O", 1), };

    protected boolean cellInterfaceDeterminesUniqueNetlistStructure() {
        return true;
    }

    public RAM64X1S_1_logic(Node parent, Wire we, Wire d, Wire wclk, Wire a0, Wire a1, Wire a2, Wire a3, Wire a4,
            Wire a5, Wire o, int init1, int init2) {
        super(parent);
        ...
        new decode6x64(this, abus, vcc(), rdec);

        for (int i = 0; i < WIDTH; i++) {
            if (((inits[i / MIN] >> i % MIN) & 1) == 1) {
                new fdpe_1(this, wclk, d, and(we, rdec.gw(i)), gnd(), regout.gw(i));
            } else {
                new fdcpe_1(this, wclk, d, and(we, rdec.gw(i)), gnd(), gnd(), regout.gw(i));
            }
