...
public interface DirectedGraph extends Serializable {
	 ...
	 * the node passed in as a parameter. Descendents are all nodes that are
	...
	/**
	 * Return a Collection of Edge objects that are "inputs" to the given
	 * node (i.e. edge goes into the node). The given node is considered the
	 * "sink" of the edge. This method will return an empty Collection if there
	 * are no input edges.
	 */
	public Collection<Edge> getInputEdges(Object node);

	/**
	 * Return a Collection of Edge objects that are "outputs" to the given
	 * node (i.e. edge goes out of the node). The given node is considered the
	...
	/**
	 * Return the Edge associated with the given source,sink pair. If there
	 * is no edge, return null.
	 * 
