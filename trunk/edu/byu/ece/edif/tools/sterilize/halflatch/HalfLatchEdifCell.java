...
public class HalfLatchEdifCell extends EdifCell {
	 ...
	 */
    public HalfLatchEdifCell(EdifLibrary lib, String name, 
			HalfLatchArchitecture hlArchitecture, int safeConstantPolarity) throws EdifNameConflictException, InvalidEdifNameException {
	 ...
	 * @throws EdifNameConflictException 
	 */
    public HalfLatchEdifCell(EdifLibrary lib, EdifNameable name, 
	 ...
	 * @throws EdifNameConflictException 
	 */
	public HalfLatchEdifCell(EdifLibrary lib, EdifCell cell, 
			HalfLatchArchitecture hlArchitecture, int safeConstantPolarity) throws EdifNameConflictException, InvalidEdifNameException {
        this(lib, cell, new RenamedObject(cell.getName()+"_hl", cell.getOldName()), hlArchitecture, safeConstantPolarity);
	 ...
	 * @throws EdifNameConflictException 
	 */
	public HalfLatchEdifCell(EdifLibrary lib, EdifCell cell, String name,
			HalfLatchArchitecture hlArchitecture, int safeConstantPolarity) throws EdifNameConflictException, InvalidEdifNameException {
	 ...
	 * @throws EdifNameConflictException 
	 */
	public HalfLatchEdifCell(EdifLibrary lib, EdifCell cell, EdifNameable name,
			HalfLatchArchitecture hlArchitecture, int safeConstantPolarity) throws EdifNameConflictException, InvalidEdifNameException {
        super(lib, cell, name);
		_hlArchitecture = hlArchitecture;
        _safeConstantPolarity = safeConstantPolarity;
        initSubCellLists();
        if (_debug) System.out.println ("Constructed new hl edif cell " + 
        		this + " with polarity " + _safeConstantPolarity);
    }

	/**
	 * Create a collection of the primitive and non-primitive sub-cell instances
	 * BHP: Include BlackBox sub-cells in the list of primitives? They should
	 *   be treated exactly the same, I believe.
	 */
	...
	/**
	 * General algorithm as developed by Keith Morgan,
	 * Nathan Rollins, Brian Pratt & Dr. Wirthlin    	
	 * 
	 * For each sub-cell instance do...
	 * 1. Fix the sub-cell instance based on the following cases...
	 *    case 1 - not primitive
	 *       i.   Modify the cell reference of each instance to point to 
	 ...
	 *            TODO: If the call to fixPotentialHalfLatches were based
	 *                  on hierarchy, this could only be done when children
	 *                  cells (no matter their level in the hierarchy) need
	 *                  the safe constant passed down.
	 *    case 2 - primitive
	 ...
	 * End For
	 * 
	 *  ** NOTE: The 'safe constant' net, inverted 'safe constant' net,
	 *           'safe constant' net inverter, and 'safe constant' port
	 *           are NOT created until they are requested for the first time.
	 */
...
*/
	/**
	 * This function modifies the passed in cell instance eci so that the
	 * cell it references (in other words, the cell of which it is an instance)
	 * is actually now the EdifHalfLatchCell version.
	 */
     ...
     * half-latch safe version.
     * i.e. An FD flip-flop gets replaced with an FDCPE
     */
    public int replaceSensitivePrimitive(EdifCellInstance eci) {
    	...
    	EdifCell safePrimitive = this.getHalfLatchArchitecture().findOrAddPrimitiveReplacementCell(elm, safePrimitiveType);
    	...
    	if (origName.endsWith(SENSITIVE_SUFFIX))
    		safePrimitiveInstanceName = origName.substring(0, 
    				origName.length()-SENSITIVE_SUFFIX.length());
    	else
    	...
    	for (int i=0; i<unconnectedPorts.length; i++) {
    		...
    		int defaultPortValue = this.getHalfLatchArchitecture().getPrimitiveReplacementFloatingPortDefaultValue(eci, unconnectedPorts[i]);
    ...
    /** 
     * Connect 'safe constant' net in the current cell to safe constant 
     * port in cell instance passed in.
     */
     ...
     * Disconnect all connections to the constant primitive.
     * Delete the constant primitive.
     */
    ...
    public int replaceConstantPrimitiveWithoutGraph(EdifCellInstance eci) {
    	...
    	for (int i=0; i<thisCellPortRefs.length; i++) {
			...
			if (epr_eci != null && eci.getName().compareToIgnoreCase(epr_eci.getName()) == 0) {
				...
				if (_debug) System.out.println("Found port ref " + ithCellEPR + " attached to constant cell " + eci);
				if (_debug) System.out.println("Net " + oldNet + " has the following connections...");
				...
				for (int j=0; j<oldNetPortRefs.length; j++) {
					...
					if (_debug) System.out.println(jthNetEPR);
					...
					if (jthNetEPR.getCellInstance() == null && 
						...
						EdifPortRef newPortRef = new EdifPortRef(this.findOrAddSafeConstantNet(constantValue), jthNetEPRSingleBitPort, null);
						this.findOrAddSafeConstantNet(constantValue).addPortConnection(newPortRef);
						if (_debug) System.out.println("Adding port ref " + newPortRef + " to safe constant net");
					...
					// attach to the constant cell instance
						...
						EdifPortRef newPortRef = new EdifPortRef(this.findOrAddSafeConstantNet(constantValue), jthNetEPRSingleBitPort, jthNetEPRCellInstance);
						this.findOrAddSafeConstantNet(constantValue).addPortConnection(newPortRef);
						if (_debug) System.out.println("Adding port ref " + newPortRef + " to safe constant net");
					}
    ...
    public int replaceConstantPrimitiveUsingGraph(EdifCellInstance eci) {
    	...
    	for (Iterator i = oldPortRefs.iterator(); i.hasNext();) {
    		...
    		// Iterate over all ports for the current net
    		// Replace the current port ref with a port ref to the 'safe constant' net
    		if (_debug) System.out.println("Found port ref " + ithEPR + " attached to constant cell " + eci);
    		if (_debug) System.out.println("Net " + oldNet + " has the following connections...");
    		Object[] oldNetPortRefs = oldNet.getConnectedPortRefs().toArray();
    		for (int j=0; j<oldNetPortRefs.length; j++) {
    			EdifPortRef jthNetEPR = (EdifPortRef) oldNetPortRefs[j];
     ...
     * The element is architecture specific (for example, in a Xilinx
     * architecture an all 0 LUT will be used).
     * Disconnect the 'safe constant' port and drive everything
     * it was driving with the new 'safe constant' generator.
     * TODO: This should probably automatically call the
     *       removeSafeConstantPortBuffer method.
     */
    public void addSafeConstantGeneratorCell() {
    	if (_safeConstantGeneratorCell == null) {
    		...
    		_safeConstantGeneratorCell = this.getHalfLatchArchitecture().addConstantCellInstance(this, getSafeConstantPolarity());
    ...
    /**
     * This method should be called on the top-level cell if
     * an internal constant cell is not going to be generated to
     * drive the safe constant distribution network.
     * The top-level port needs to be buffered so it will be kept.
     */
    public void bufferSafeConstantPort() {
    	if (_safeConstantGeneratorCell != null)
    		throw new EdifRuntimeException("Error: Cannot buffer safe-constant port in cell which already had port removed and internal constant generator source added.");
    	...
    	if (_inputBufferCellInstance == null) {
    		...
    		if (_safeConstantPolarity == 0) constantPortToBufferNetName = _safeConstantZeroNetName + "_i"; 
    		else constantPortToBufferNetName = _safeConstantOneNetName + "_i";
    		// create a net to connect the top-level port to the ibuf
    		...
    		EdifSingleBitPort inputBufferInputPort = inputBufferCell.getPort(this.getHalfLatchArchitecture().getPrimitiveInputBufferCellInputPortName()).getSingleBitPort(0);
    		// make a port ref object to connect the port to the net
    		...
    		EdifSingleBitPort inputBufferOutputPort = inputBufferCell.getPort(this.getHalfLatchArchitecture().getPrimitiveInputBufferOutputBufferName()).getSingleBitPort(0);
    		// make a port ref object to connect the port to the net
    		// add the port ref to the net
    		this.findOrAddSafeConstantNet().addPortConnection(new EdifPortRef(this.findOrAddSafeConstantNet(), inputBufferOutputPort, _inputBufferCellInstance));
    ...
    /**
     * This method should NOT be called directly.  It is meant only
     * as a utility function for findOrAddSafeConstantPort().
     * This method pre-determines the safeConstantPort's name.
     * If the safe constant port for 'this' cell has not already been
     * created and added, then this method call will first create
     * and add the port to 'this' cell before returning a reference to it.
     */
    private EdifPort addSafeConstantPort() {
    	EdifPort result;
    	if (_safeConstantPort == null) {
    		String portName = null;
    		if (_debug) System.out.println("Adding safe constant port to " + this + " with polarity " + _safeConstantPolarity);
    		if (_safeConstantPolarity == 0) portName = _safeConstantZeroPortName;
    		else portName = _safeConstantOnePortName;
    ...
    /**
     * This method should NOT be called directly.  It is meant only
     * as a utility function for addSafeConstantPort().
     * If the safe constant port for 'this' cell has not already been
     * created and added, then this method call will first create
     * and add the port to 'this' cell before returning a reference to it.
     */
    private EdifPort addSafeConstantPort(String portName) {
    	if (_safeConstantPort == null) {
    		if (portName == null) throw new EdifRuntimeException("String for new safe constant port is null");
    		try {
			...
			_safeConstantNetSafeConstantPortEPR = new EdifPortRef(this.findOrAddSafeConstantNet(), thisSafeConstantPort, null);
...
/*    		EdifNet connectedNet = this.getSafeConstantPort().getInnerNet();
    		EdifPortRef safeConstantPortSafeConstantNetEPR = connectedNet.getEdifPortRef(null, _safeConstantPort.getSingleBitPort(0));
...
*/    		
    ...
    public EdifNet getSafeConstantNet(int polarity) {
    	EdifNet result;
    	if (polarity == 0 && _safeConstantPolarity == 0 ||
    	    polarity == 1 && _safeConstantPolarity == 1)
     ...
     * It is meant as a utility function for the function
     * findOrAddSafeConstantNetNormPolarity()
     */
    private EdifNet addSafeConstantNetNormPolarity() {		
    	if (_safeConstantNetNorm == null) {
    		String netName = null;
    		if (_safeConstantPolarity == 0) netName = _safeConstantZeroNetName; 
    		else netName = _safeConstantOneNetName;
			try {
     ...
     * polarity for 'this' cell have not already been created and added, 
     * then this method call will first create and add the inverter and
     * net (attached to the output of the inverter) to 'this' cell before
     * returning a reference to the inverted polarity net.
     */
    ...
    private EdifNet addSafeConstantNetInvPolarity() {
		if (_safeConstantNetInv == null) {
    		String netName = null;
    		if (_safeConstantPolarity == 0) netName = _safeConstantOneNetName;
    		else netName = _safeConstantZeroNetName;
    		try {
     ...
     * If the safe constant net with *normal* polarity for 'this' cell has not 
     * already been created and added, then this method call will first create
     * and add the net to 'this' cell.  By default, this method will create
     * a net which is attached to the output of the inverter this method
     * is supposed to create.
     */
    private void addSafeConstantInverter() {
    	if (_safeConstantInverterInstance == null) { 
    		// create an inverter
    		EdifCell inverterCell = this.getHalfLatchArchitecture().findOrAddPrimitiveInverterCell(this.getLibrary().getLibraryManager());
    		try {
    		...
    		EdifSingleBitPort inverterInputPort = inverterCell.getPort(this.getHalfLatchArchitecture().getPrimitiveInverterCellInputPortName()).getSingleBitPort(0);
    		EdifPortRef constantNetInverterInputEPR = new EdifPortRef(thisSafeConstantNet, inverterInputPort, _safeConstantInverterInstance);
    		...
    		EdifSingleBitPort inverterOutputPort = inverterCell.getPort(this.getHalfLatchArchitecture().getPrimitiveInverterCellOutputPortName()).getSingleBitPort(0);
    		EdifPortRef constantInvNetInverterOutputEPR = new EdifPortRef(thisSafeConstantNetInv, inverterOutputPort, _safeConstantInverterInstance);
    ...
    public EdifPortRef findOrAddSafeConstantNetSafeConstantPortEPR() {
    	if (_safeConstantNetSafeConstantPortEPR == null) {
    		if (_safeConstantNetNorm != null) {
    			_safeConstantNetNorm.addPortConnection(new EdifPortRef(this.findOrAddSafeConstantNet(), this.findOrAddSafeConstantPort().getSingleBitPort(0), null));
    ...
    public static int test(String inputEdifFilename, String outputEdifFilename) {
        ...
        SequentialEdifHalfLatchRemover sequentialEdifHalfLatchRemover = new SequentialEdifHalfLatchRemover(hlArchitecture, safeConstantPolarity, true);
            	...
            	if (cell.isPrimitive() == false) {
            		if (testHasSafeNet(cell, safeConstantNetName) == true) {
                        if (testHasSafePort(cell, safeConstantPortName) == false)
                        	error_report += ("\n Error: HalfLatchEdifCell: Cell " + cell.getName() + " has no safe constant port, but it should have one named " + safeConstantPortName);
                        ...
                        for (Iterator m = cell.getSubCellList().iterator(); m.hasNext();) {
                        	...
                        	if ( (subcell_eci.getCellType().isPrimitive() == false) && (testHasSafePort(subcell_eci.getCellType(), safeConstantPortName) == false) )
                        		nonConnectedNonPrimitiveSubCells.add(subcell_eci);
                        }
                        if (nonConnectedNonPrimitiveSubCells.size() > 0)
                        	error_report += ("\n Error: HalfLatchEdifCell: Cell " + cell.getName() + " has the following non-primitive sub-cell instances with no safe constant port... " + nonConnectedNonPrimitiveSubCells);
            		}
            		else {
            			error_report += ("\n Error: HalfLatchEdifCell: Cell " + cell.getName() + " has no safe constant net, but it should have one named " + safeConstantNetName);
            		}
            	}
        	}
        }
        	...
        	if (cellInstances != null) {
        		for (Iterator j=cellInstances.iterator(); j.hasNext();) {
        			...
        			if (hlArchitecture.isConstantCell(eci.getType()) == true)
        				error_report += ("\n Error: HalfLatchEdifCell: Design has remaining constant cell instance " + eci.getName());
        			else if (hlArchitecture.cellRequiresReplacement(eci) == true)
        				error_report += ("\n Error: HalfLatchEdifCell: Design has remaining half-latch prone primitive cell instance " + eci.getName());        		
        		}
        	}
        }
