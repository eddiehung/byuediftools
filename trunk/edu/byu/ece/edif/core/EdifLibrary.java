...
public class EdifLibrary extends NamedPropertyObject implements EdifOut, Trimable {
    ...
    /**
     * Two EdifLibrary objects are considered edifEquals if:
     * <ul>
     * <li> Each of the cells in the EdifLibrary are edifEqual</li>
     * <li> Both EdifLibrary objects have the same name (as defined by
     * getName().equals()</li>
     * <li> Both EdifLibrary objects have the same properties (or both have no
     * properties)</li>
     * <li> Both EdifLibrary objects are external or both are not external
     * libraries </li>
     * </ul>
     * <p>
     ...
     * 
     * @param o Comparison EdifLibrary object
     * @return true of the two EdifLibrary objects are edifEqual
     */
    public boolean edifEquals(Object o) {
        if (this == o)
            return true;
        if (!(o instanceof EdifLibrary))
            return false;

        EdifLibrary that = (EdifLibrary) o;

        return this.isExternal() == that.isExternal()
                ...
                && (this.getPropertyList() == null ? that.getPropertyList() == null : this.getPropertyList().equals(
                        that.getPropertyList())) && edifEqualsCellLists(that);
    }
    ...
    public List<EdifDifference> edifDiff(Object o) {
        ...
        if (this.isExternal() != that.isExternal())
            differences.add(new EdifDifference("isExternal: " + this.isExternal(), "isExternal: " + that.isExternal()));
        ...
        if (!this.edifEqualsCellLists(that)) {
            differences.add(new EdifDifference(this.getCells(), that.getCells()));
            differences.addAll(this.edifDiffCellLists(that));
        }
    ...
    /**
     * Two EdifCell lists are considered edifEqual if
     * <ul>
     * <li> Each of the EdifCells objects are edifEqual</li>
     * </ul>
     * 
     * @param that Comparison EdifLibrary object
     * @return true if the EdifCell lists are edifEqual
     */
    private boolean edifEqualsCellLists(EdifLibrary that) {
        Collection<EdifCell> thisCellList = this.getCells();
        Collection<EdifCell> thatCellList = that.getCells();

        if (thisCellList == null && thatCellList != null)
            return false;
        if (thisCellList != null && thatCellList == null)
            return false;
        ...
        if (thisCellList != null)
            for (EdifCell thisCell : thisCellList) {
                boolean result = false;
                for (EdifCell thatCell : thatCellList) {
                    if (thisCell.edifEquals(thatCell)) {
                        result = true;
                        break;
                    }
                }
     ...
     * @return true if the EdifCell lists are edifEqual
     */
    private List<EdifDifference> edifDiffCellLists(EdifLibrary that) {
        List<EdifDifference> differences = new ArrayList<EdifDifference>();

        Collection<EdifCell> thisCellList = this.getCells();
        Collection<EdifCell> thatCellList = that.getCells();

        if ((thisCellList == null && thatCellList != null) || (thisCellList != null && thatCellList == null)) {
            differences.add(new EdifDifference(thisCellList, thatCellList));
            return differences;
        }
        ...
        if (thisCellList != null)
            for (EdifCell thisCell : thisCellList) {
                ...
                if (result == false) {
                    differences.add(new EdifDifference(thisCell, " Not found in library: " + that.getName()));
                }
            }

        if (thatCellList != null)
            for (EdifCell thatCell : thatCellList) {
                boolean result = false;
                for (EdifCell thisCell : thisCellList) {
                    if (thatCell.edifEquals(thisCell)) {
                        result = true;
                        break;
                    }
                }
                if (result == false) {
                    differences.add(new EdifDifference(" Not found in library: " + this.getName(), thatCell));
                }
            }
