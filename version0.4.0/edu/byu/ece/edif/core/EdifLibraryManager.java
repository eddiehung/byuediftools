...
public class EdifLibraryManager implements Trimable, EdifOut, Serializable {
    ...
    /**
     * Two EdifLibraryManager objects are considered edifEquals if:
     * <ul>
     * <li> Each of the EdifLibrary objects in the EdifLibraryManager objects
     * are edifEqual</li>
     * </ul>
     * TODO: Currently, the code that compares the EdifCell objects will only
     * check to see if one set of EdifCell objects is a subset of the other.
     * Change the code so that it checks to make sure that both sets are equal.
     * One way to do this is to get a copy List of the EdifCell objects and, as
     * each match is found between the two libraries, remove the cell from each
     * List. After the first list is empty, the sets are equal if and only if
     * the second List is also empty. --jamesfcarroll
     * 
     * @param o Comparison EdifLibraryManager object
     * @return true of the two EdifLibraryManager objects are edifEqual
     */
    public boolean edifEquals(Object o) {
        if (this == o)
            return true;
        if (!(o instanceof EdifLibraryManager))
            return false;

        EdifLibraryManager that = (EdifLibraryManager) o;

        return edifEqualsLibraryList(that);
    }
    ...
    public List<EdifDifference> edifDiff(Object o, boolean verbose) {
        ...
        if (!this.edifEqualsLibraryList(that)) {
            differences.add(new EdifDifference(this, that));
        }

        if (!verbose)
            return differences;

        differences.addAll(this.edifDiffLibraryList(that));
    ...
    /**
     * Re-order the libraries and the cells within into a proper order (if they
     * aren't already).
     */
    //    public void validateOrder() { 
    //        // if(isValid()) return; 
    //        Collection reAddLibs = _libraries;
    //        _libraries = new ArrayListNameSpaceResolver();
    //        for (Iterator i = reAddLibs.iterator(); i.hasNext();) {
    ...
    ////                         private methods                   ////
    /**
     * Two EdifLibrary lists are considered edifEqual if
     * <ul>
     * <li> Each of the EdifLibrary objects are edifEqual</li>
     * </ul>
     * 
     * @param that Comparison EdifLibraryManager object
     * @return true if the EdifLibrary lists are edifEqual
     */
    private boolean edifEqualsLibraryList(EdifLibraryManager that) {
        List<EdifLibrary> thisLibraryList = this.getLibraries();
        List<EdifLibrary> thatLibraryList = that.getLibraries();

        if (thisLibraryList == null && thatLibraryList != null)
            return false;
        if (thisLibraryList != null && thatLibraryList == null)
            return false;

        // Make sure that each of the EdifCell objects in this EdifLibrary match
        // one of the EdifCell in that EdifLibrary.
        if (thisLibraryList != null)
            for (EdifLibrary thisLibrary : thisLibraryList) {
                boolean result = false;
                for (EdifLibrary thatLibrary : thatLibraryList) {
                    if (thisLibrary.edifEquals(thatLibrary)) {
                        result = true;
                        break;
                    }
                }
    ...
    private List<EdifDifference> edifDiffLibraryList(EdifLibraryManager that) {
        List<EdifDifference> differences = new ArrayList<EdifDifference>();

        List<EdifLibrary> thisLibraryList = new ArrayList<EdifLibrary>(this.getLibraries());
        List<EdifLibrary> thatLibraryList = new ArrayList<EdifLibrary>(that.getLibraries());

        if ((thisLibraryList == null && thatLibraryList != null)
                || (thisLibraryList != null && thatLibraryList == null)) {
            differences.add(new EdifDifference(thisLibraryList, thatLibraryList));
            return differences;
        }

        // Make sure that each of the EdifCell objects in this EdifLibrary match
        // one of the EdifCell in that EdifLibrary.
        Iterator<EdifLibrary> thisI = thisLibraryList.iterator();
        while (thisI.hasNext()) {
            EdifLibrary thisLibrary = thisI.next();
            Iterator<EdifLibrary> thatI = thisLibraryList.iterator();
            while (thatI.hasNext()) {
                EdifLibrary thatLibrary = thatI.next();
                if (thisLibrary.getName().equals(thatLibrary.getName())) {
                    differences.addAll(thisLibrary.edifDiff(thatLibrary));
                    thisI.remove();
                    thatI.remove();
                    break;
                }
        ...
        // If there are any Libraries left, simply add their names to the list of differences 
        for (EdifLibrary thisLibrary : thisLibraryList) {
            differences.add(new EdifDifference(thisLibrary.getName(), ""));
        }
        for (EdifLibrary thatLibrary : thatLibraryList) {
            differences.add(new EdifDifference("", thatLibrary.getName()));
        }
