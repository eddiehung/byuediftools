...
public class RAM16X8D_logic extends Logic {
    public static CellInterface cell_interface[] = { in("WE", 1), in("D", 8), in("WCLK", 1), in("A0", 1), in("A1", 1),
            in("A2", 1), in("A3", 1), in("DPRA0", 1), in("DPRA1", 1), in("DPRA2", 1), in("DPRA3", 1), out("SPO", 8),
            out("DPO", 8), };
    ...
    public RAM16X8D_logic(Node parent, Wire we, Wire d, Wire wclk, Wire a0, Wire a1, Wire a2, Wire a3, Wire dpra0,
            Wire dpra1, Wire dpra2, Wire dpra3, Wire spo, Wire dpo, int init1, int init2, int init3, int init4,
            int init5, int init6, int init7, int init8) {
        super(parent);
