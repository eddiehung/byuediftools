...
public class RAM16X4D_logic extends Logic {
    public static CellInterface cell_interface[] = { in("WE", 1), in("D0", 1), in("D1", 1), in("D2", 1), in("D3", 1),
            in("WCLK", 1), in("A0", 1), in("A1", 1), in("A2", 1), in("A3", 1), in("DPRA0", 1), in("DPRA1", 1),
            in("DPRA2", 1), in("DPRA3", 1), out("SPO0", 1), out("SPO1", 1), out("SPO2", 1), out("SPO3", 1),
            out("DPO0", 1), out("DPO1", 1), out("DPO2", 1), out("DPO3", 1), };

    protected boolean cellInterfaceDeterminesUniqueNetlistStructure() {
        return true;
    }

    public RAM16X4D_logic(Node parent, Wire we, Wire d0, Wire d1, Wire d2, Wire d3, Wire wclk, Wire a0, Wire a1,
            Wire a2, Wire a3, Wire dpra0, Wire dpra1, Wire dpra2, Wire dpra3, Wire spo0, Wire spo1, Wire spo2,
            Wire spo3, Wire dpo0, Wire dpo1, Wire dpo2, Wire dpo3, int init1, int init2, int init3, int init4) {
        super(parent);
