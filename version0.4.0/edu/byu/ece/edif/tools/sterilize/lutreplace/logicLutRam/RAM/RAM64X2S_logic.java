/*
 * Logic implementation of a RAM64X2S
 * 
...
public class RAM64X2S_logic extends Logic {
    public static CellInterface cell_interface[] = { in("WE", 1), in("D0", 1), in("D1", 1), in("WCLK", 1), in("A0", 1),
            in("A1", 1), in("A2", 1), in("A3", 1), in("A4", 1), in("A5", 1), out("O0", 1), out("O1", 1), };

    protected boolean cellInterfaceDeterminesUniqueNetlistStructure() {
        return true;
    }

    public RAM64X2S_logic(Node parent, Wire we, Wire d0, Wire d1, Wire wclk, Wire a0, Wire a1, Wire a2, Wire a3,
            Wire a4, Wire a5, Wire o0, Wire o1, int init1, int init2, int init3, int init4) {
        super(parent);
